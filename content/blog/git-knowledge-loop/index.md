...
```text
<类型>(<可选影响范围>): <72 字以内的祈使语气摘要>

<一到五行祈使语气的说明，阐述任务内容与动机>
...
1. 可用时优先用 `rg` 或 `rg --files` 定位相关符号、调用方、测试和可能的路径；没有则使用等效的仓库搜索工具。在打开大范围源文件之前，先确定可能的改动边界。
2. 只通过 `.codex/MODULES.md` 解析本次改动实际涉及的模块；ID 中的 `/` 对应子目录。
3. 对每个受影响的模块，读取存在的 `.codex/maps/<module>.md` 与 `.codex/decisions/<module>.md`。每个必读的知识文件都要单独读完整，文件过大时分块读取。不要加载无关模块的文件。
4. 先查看确切的定义、直接调用方、直接被调用方以及相关测试。只有为了回答某个明确的未决问题，才扩展到相邻文件、模块或依赖源码。
5. 对即将改动的文件运行 `git log --oneline -10 -- <path>`。
6. 只用 `git show` 查看与当前行为相关的近期提交，包括匹配 `MODULE: <当前模块>` 的 Decision 块。

上下文加载护栏：

- 不要把多个大型知识文件或源文件拼接进同一条命令。
- 若输出被截断，不得视为该文件已读完；改用聚焦的行区间重复读取，直到覆盖所需内容。
- 一旦行为、改动边界、约束和验证路径都已确立，就停止继续加载上下文。

### 完成任务之后

- 如果实现改动导致现有模块 map 在职责、公开入口点、生命周期或数据流、依赖、不变量、已知局限方面失准，在可用时以定向刷新模式使用 `$map-module`；否则在收尾前报告该 map 已过期。影响范围无法界定时使用全量刷新。若 map 中记录的陈述依然成立，则不要改动它。
