...
1. 通过 `.codex/MODULES.md` 解析本次变更涉及的每一个模块；ID 中的 `/` 对应到子目录。
2. 对每个受影响的模块，若存在 `.codex/maps/<module>.md` 和 `.codex/decisions/<module>.md`，则读取它们。
   不要加载无关的模块文件；如果输出被截断，继续按聚焦的行范围读取，直到每个必需文件都读完整。
3. 只有为了回答某个明确的未解决问题，才扩展源码上下文。一旦行为、变更边界、约束和验证路径都清楚了，就停止。
4. 对即将修改的文件运行 `git log --oneline -10 -- <path>`。
   仅对与当前行为相关的近期提交使用 `git show`，
   包括那些带有匹配 `MODULE: <current module>` Decision 块的提交。

### 完成任务之后

- 如果实现变更使现有的模块地图失效，使用 `$map-module`：影响范围有限时做定向刷新，否则做完整刷新。
  若该技能不可用，则报告该地图已过时。
- 在每个 `$commit-context` Decision 中，
  填写 `MODULE`、`WHY`、`ALTERNATIVES`、`CHOSEN`、`TRADEOFFS` 和 `RISKS`；
  当替换先前的 Decision 时，补充 `SUPERSEDES`。
```
