...
首次运行需要等待较长时间，如果你的项目历史比较悠久，甚至可能需要运行好几个小时，所以推荐在下班时运行，第二天早上上班时验收。考虑到一个代码仓库有外部依赖，必要时你可以在 prompt 里提供各个依赖的源码路径，`$map-module` 会自动探索对应的路径，并将结论沉淀到 `.codex/maps/<id>.md` 中，格式大概是这样的：
